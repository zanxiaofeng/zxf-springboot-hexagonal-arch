...
**版本：** 1.4（2026-08-22 通用化：§2.1 的模式 A 采用标注移出——正文声明「各项目按判据选定并记录于项目 CLAUDE.md」，§3 起示例仍按模式 A 书写并注明替换关系；§6.1/§6.2 信封表述统一 `errors[]` 标准结构；项目特定路径指称移除。此前 1.3：新增 §2.1 业务异常表达策略）
